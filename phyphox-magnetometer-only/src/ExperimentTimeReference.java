...
public class ExperimentTimeReference implements Serializable {
    ...
    public enum TimeMappingEvent {
        START, PAUSE, CLEAR // Note about clear: Since the timemapping list is also cleared in that case,
                            // clear is immediately removed after being added to the list. It is however
                            // transmitted to clients via the event BLE characteristic
    }
    ...
    public void logToDebug() {
        for (TimeMapping mapping : timeMappings) {
            Log.d("TimeReference", mapping.event.name() + ": experiment time = " + mapping.experimentTime
                    + ", event time = " + mapping.eventTime + ", system time = " + mapping.systemTime);
        }
        Log.d("TimeReference", "...");
    }
            ...
            switch (last.event) {
                case START:
                    if (event == TimeMappingEvent.START)
                        return;
                    timeMappings
                            .add(new TimeMapping(event, getExperimentTimeFromEvent(eventTime), eventTime, systemTime));
                    break;
                case PAUSE:
    ...
    public int getReferenceIndexFromLinearTime(double t) {
        int i = 0;
        while (timeMappings.size() > i + 1
                && (timeMappings.get(i + 1).systemTime - timeMappings.get(0).systemTime) * 0.001 <= t) {
            i++;
        }
        return i;
    }
