    ...
    private void handleClient(Socket client) {
        new Thread(() -> {
            try {
                ...
                if (line.contains("GET /get?magnetometer")) {
                    // Check for target parameter in the URL
                    // Example: GET /get?magnetometer&target=150.5 HTTP/1.1
                    if (magnetometerInput != null && line.contains("&target=")) {
                        try {
                            String[] parts = line.split("&target=");
                            if (parts.length > 1) {
                                String valStr = parts[1].split(" ")[0]; // Get value before space/HTTP
                                double val = Double.parseDouble(valStr);
                                magnetometerInput.targetMT = val;
                            }
                        } catch (Exception e) {
                            // Ignore parsing errors, keep old target
                        }
                    }
