...
var PhyphoxManager = {
	...
	targetReachedTime: null, // Track when target was reached
	holdDuration: 2000, // Hold target for 2 seconds before stopping
	...
	setAutoAdapt: function (val) {
		...
		// Reset target reached time when enabling/disabling
		if (!val) {
	...
	doAdaptation: function () {
		...
		// Check if we're within the target deadband
		if (Math.abs(diff) < deadband) {
			// Target reached - start or continue the hold timer
			var now = Date.now();
			if (this.targetReachedTime === null) {
				// Just reached target - start the timer
				this.targetReachedTime = now;
			} else {
				// Already at target - check if we've held for 2 seconds
				var timeAtTarget = now - this.targetReachedTime;
				if (timeAtTarget >= this.holdDuration) {
					// Held at target for 2 seconds - stop auto-adapting
					this.setAutoAdapt(false);
					return;
				}
			}
			// Still holding at target, don't adjust
			return;
		} else {
			// Not at target - reset the timer and continue adjusting
			this.targetReachedTime = null;
		}
		
		// Adjust volume to reach target
		var pGain = 0.00015;
